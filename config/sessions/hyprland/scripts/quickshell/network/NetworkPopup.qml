...
Item {
    ...
    readonly property color vpnAccent: "#a6e3a1"
    ...
    property string connResult: "idle"   // idle | connecting | success | error
    property string connErrorMsg: ""
    property string connProcessSsid: ""
    property var ethernetInfo: null
    property string vpnConnResult: "idle"   // idle | connecting | connected | error
    property string vpnErrorMsg: ""
    ...
    // ── WiFi connect process (captures result for feedback) ──────────────
    Process {
        id: wifiConnectProcess
        stdout: StdioCollector {
            onStreamFinished: {
                let out = this.text.trim();
                if (out.includes("__WIFI_OK__")) {
                    window.connResult = "success";
                    let bt = window.busyTasks;
                    delete bt[window.connProcessSsid];
                    window.busyTasks = Object.assign({}, bt);
                    connSuccessClose.start();
                } else {
                    window.connResult = "error";
                    if (out.includes("password") || out.includes("Secrets") || out.includes("802-11") || out.includes("activation failed")) {
                        window.connErrorMsg = "Wrong password";
                    } else {
                        window.connErrorMsg = "Connection failed";
                    }
                    let bt = window.busyTasks;
                    delete bt[window.connProcessSsid];
                    window.busyTasks = Object.assign({}, bt);
                    window.showPasswordDialog = true;
                }
    ...
    Timer { id: connSuccessClose; interval: 1200; onTriggered: { window.connResult = "idle"; window.showPasswordDialog = false; } }

    // ── Ethernet poller ───────────────────────────────────────────────────
    Process {
        id: ethernetPoller
        command: ["bash", "-c",
            "nmcli -t -f DEVICE,TYPE,STATE,IP4.ADDRESS device show 2>/dev/null | " +
            "awk -F: '/^GENERAL.TYPE:ethernet/{found=1} found && /^GENERAL.STATE:/{state=$2} found && /^IP4.ADDRESS/{ip=$2} found && /^GENERAL.DEVICE/{dev=$2} END{if(found && state~/connected/) print dev"|"ip}' | head -1 || true"]
        running: true
        stdout: StdioCollector {
            onStreamFinished: {
                let out = this.text.trim();
                if (out !== "") {
                    let parts = out.split("|");
                    window.ethernetInfo = { device: parts[0] || "eth", ip: (parts[1] || "").replace(/\/[0-9]+/, "") };
                } else {
                    window.ethernetInfo = null;
                }
            }
        }
    }

    Timer { interval: 5000; running: true; repeat: true; onTriggered: ethernetPoller.running = true; }
    // ── VPN status poller ────────────────────────────────────────────────
    Process {
        id: vpnPoller
        command: ["bash", "-c", "nmcli -t -f NAME,TYPE,STATE connection show --active 2>/dev/null | grep ':vpn:' | cut -d: -f1 | head -1"]
        running: true
        stdout: StdioCollector {
            onStreamFinished: {
                let out = this.text.trim();
                if (out.length > 0) {
                    if (window.vpnConnResult !== "connecting") window.vpnConnResult = "connected";
                } else {
                    if (window.vpnConnResult !== "connecting") window.vpnConnResult = "idle";
                }
            }
        }
    }
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                let out = this.text.trim();
                if (out.includes("__VPN_OK__")) {
                    window.vpnConnResult = "connected";
                    window.showVpnDialog = false;
                    window.vpnErrorMsg = "";
                } else if (out.includes("__VPN_DOWN__")) {
                    window.vpnConnResult = "idle";
                } else {
                    window.vpnConnResult = "error";
                    if (out.includes("No agents") || out.includes("no secrets") || out.includes("password") || out.includes("Auth") || out.includes("auth")) {
                        window.vpnErrorMsg = "";
                        window.showVpnDialog = true;
                    } else {
                        window.vpnErrorMsg = "Connection failed";
                        window.showVpnDialog = true;
                    }
                }
    ...
    Item {
        ...
        Rectangle {
                                ...
                                NumberAnimation {
                                    ...
                                    onFinished: {
                                        ...
                                        if (cmdStr === "TOGGLE_VIEW") {
                                            ...
                                            if (window.activeMode === "wifi" && security !== "" && security !== "--") {
                                                // Secured network — show password dialog
                                                window.pendingSsid = ssid;
                                                window.showPasswordDialog = true;
                                                floatCard.triggered = false;
                                                drainAnim.start();
                                            } else {
            ...
            Rectangle {
                ...
                height: 38
                radius: 12
                color: window.vpnConnResult === "connected" ? "#18a6e3a1" : "#14ffffff"
                border.color: window.vpnConnResult === "connected" ? window.vpnAccent : "#33ffffff"
                border.width: 1
                Behavior on color { ColorAnimation { duration: 300 } }
                Behavior on border.color { ColorAnimation { duration: 300 } }
                z: 10
                ...
                RowLayout {
                    ...
                    Text {
                        font.family: "Iosevka Nerd Font"
                        font.pixelSize: 16
                        color: window.vpnConnResult === "connected" ? window.vpnAccent : window.subtext0
                        text: "󰦝"
                        Behavior on color { ColorAnimation { duration: 300 } }
                    }
                    ...
                    Text {
                        ...
                        font.family: "JetBrains Mono"
                        font.pixelSize: 11
                        color: window.vpnConnResult === "connected" ? window.text : window.subtext0
                        text: "Ensimag VPN  " + (window.vpnConnResult === "connected" ? "Connected" : window.vpnConnResult === "connecting" ? "Connecting..." : window.vpnConnResult === "error" ? window.vpnErrorMsg : "Disconnected")
                        Behavior on color { ColorAnimation { duration: 300 } }
                        elide: Text.ElideRight
                    }
                    ...
                    Rectangle {
                        width: 80; height: 26
                        radius: 8
                        color: window.vpnConnResult === "connected"
                            ? (vpnBtnMa.containsMouse ? Qt.rgba(window.red.r, window.red.g, window.red.b, 0.8) : Qt.rgba(window.red.r, window.red.g, window.red.b, 0.6))
                            : (vpnBtnMa.containsMouse ? Qt.lighter(window.vpnAccent, 1.1) : window.vpnAccent)
                        Behavior on color { ColorAnimation { duration: 150 } }
                        visible: window.vpnConnResult !== "connecting"

                        Text {
                            anchors.centerIn: parent
                            text: window.vpnConnResult === "connected" ? "Disconnect" : "Connect"
                            font.family: "JetBrains Mono"
                            font.pixelSize: 10
                            font.bold: true
                            color: window.crust
                        ...
                        MouseArea {
                            ...
                            onClicked: {
                                if (window.vpnConnResult === "connected") {
                                    window.vpnConnResult = "connecting";
                                    vpnProcess.command = ["bash", "-c", "nmcli connection down 'Ensimag-VPN-ETU-udp' 2>&1 && echo __VPN_DOWN__ || echo __VPN_FAIL__"];
                                    vpnProcess.running = true;
                                } else {
                                    // Try direct connect first (uses stored credentials)
                                    window.vpnErrorMsg = "";
                                    vpnProcess.command = ["bash", "-c", "nmcli connection up 'Ensimag-VPN-ETU-udp' 2>&1 && echo __VPN_OK__ || echo __VPN_FAIL__"];
                                    vpnProcess.running = true;
                                }
                            }
                        }
                ...
                // Indeterminate progress bar at bottom when connecting
                Rectangle {
                    visible: window.vpnConnResult === "connecting"
                    anchors.bottom: parent.bottom
                    anchors.left: parent.left
                    ...
                    Rectangle {
                        ...
                        radius: 2
                        width: 60
                        color: window.vpnAccent

                        SequentialAnimation {
                            running: window.vpnConnResult === "connecting"
                            loops: Animation.Infinite
                            NumberAnimation { target: vpnProgressBar; property: "x"; from: -60; to: 340; duration: 1200; easing.type: Easing.InOutSine }
                            NumberAnimation { target: vpnProgressBar; property: "x"; from: 340; to: -60; duration: 1200; easing.type: Easing.InOutSine }
                        }
                    }
                }
            }
    ...
    // =========================================================
    // ETHERNET BANNER (shown at top when ethernet is active)
    // =========================================================
    Rectangle {
        visible: window.ethernetInfo !== null
        anchors.top: parent.top
        anchors.left: parent.left
        anchors.right: parent.right
        anchors.margins: 12
        height: 38
        radius: 10
        color: "#1a" + window.sapphire.toString().replace("#","")
        border.color: Qt.lighter(window.sapphire, 1.1)
        border.width: 1
        z: 50
        ...
        Row {
            ...
            Text {
                anchors.verticalCenter: parent.verticalCenter
                text: "Ethernet" + (window.ethernetInfo && window.ethernetInfo.ip !== "" ? "  " + window.ethernetInfo.ip : "")
                font.family: "JetBrains Mono"
                font.pixelSize: 12
                color: window.text
            }
    ...
    Rectangle {
        id: passwordOverlay
        visible: window.showPasswordDialog || window.connResult === "connecting"
        anchors.fill: parent
        color: "#aa000000"
        z: 999
        ...
        MouseArea {
            anchors.fill: parent
            enabled: window.connResult !== "connecting"
            onClicked: {
                window.showPasswordDialog = false;
                window.connResult = "idle";
                passwordInput.text = "";
            }
        }
        ...
        Rectangle {
            anchors.centerIn: parent
            width: 320
            height: connResultRow.visible ? 230 : 200
            Behavior on height { NumberAnimation { duration: 150 } }
            radius: 16
            color: window.base
            border.color: window.connResult === "error" ? window.red
                         : window.connResult === "success" ? window.green
                         : window.wifiAccent
            border.width: 1
            ...
            Column {
                ...
                Text {
                    width: parent.width
                    text: window.connResult === "connecting" ? "Connecting..."
                         : window.connResult === "success" ? "Connected!"
                         : "Connect to " + window.pendingSsid
                    color: window.connResult === "success" ? window.green
                         : window.connResult === "error" ? window.red
                         : window.text
                    font.family: "JetBrains Mono"
                    font.pixelSize: 14
                    font.bold: true
                ...
                Rectangle {
                    ...
                    height: 28
                    radius: 6
                    visible: window.connResult === "error" || window.connResult === "success"
                    color: window.connResult === "error" ? window.surface0 : window.surface0
                    border.color: window.connResult === "error" ? window.red : window.green
                    border.width: 1
                    Text {
                        anchors.centerIn: parent
                        text: window.connResult === "error" ? window.connErrorMsg : "Network activated"
                        color: window.connResult === "error" ? window.red : window.green
                        font.family: "JetBrains Mono"
                        font.pixelSize: 12
                    }
                }
                ...
                Rectangle {
                    ...
                    border.color: passwordInput.activeFocus ? window.wifiAccent : window.surface2
                    border.width: 1
                    ...
                    Row {
                        ...
                        TextInput {
                            ...
                            font.family: "JetBrains Mono"
                            font.pixelSize: 13
                            echoMode: showPassBtn.showPass ? TextInput.Normal : TextInput.Password
                            focus: window.showPasswordDialog
                            Keys.onReturnPressed: pwdConnectBtn.doConnect()
                        ...
                        Text {
                            ...
                            MouseArea {
                                anchors.fill: parent
                                onClicked: parent.showPass = !parent.showPass
                            }
                        }
                    }
                }
                ...
                Row {
                    ...
                    Rectangle {
                        ...
                        width: 90; height: 36
                        radius: 10
                        color: connectMa.containsMouse ? Qt.lighter(window.wifiAccent, 1.15) : window.wifiAccent
                        Behavior on color { ColorAnimation { duration: 150 } }

                        function doConnect() {
                            let ssid = window.pendingSsid;
                            let pwd = passwordInput.text.trim();
                            window.connResult = "connecting";
                            window.connErrorMsg = "";
                            window.connProcessSsid = ssid;
                            let bt2 = window.busyTasks;
                            bt2[ssid] = true;
                            window.busyTasks = Object.assign({}, bt2);
                            busyTimeout.restart();
                            let cmd = pwd !== ""
                                ? "nmcli connection delete '" + ssid + "' 2>/dev/null; nmcli device wifi connect '" + ssid + "' password '" + pwd + "' 2>&1 && echo __WIFI_OK__ || echo __WIFI_FAIL__"
                                : "nmcli connection up '" + ssid + "' 2>&1 && echo __WIFI_OK__ || echo __WIFI_FAIL__";
                            wifiConnectProcess.command = ["sh", "-c", cmd];
                        ...
                        MouseArea {
                            ...
                            anchors.fill: parent
                            hoverEnabled: true
                            onClicked: pwdConnectBtn.doConnect()
                        }
                    }
                }
            }
    ...
    // =========================================================
    // VPN CREDENTIALS DIALOG
    // =========================================================
    Rectangle {
        id: vpnOverlay
        visible: window.showVpnDialog
        anchors.fill: parent
        color: "#aa000000"
        z: 998
        ...
        Rectangle {
            anchors.centerIn: parent
            width: 320
            height: window.vpnErrorMsg !== "" ? 270 : 240
            Behavior on height { NumberAnimation { duration: 150 } }
            radius: 16
            color: window.base
            border.color: window.vpnConnResult === "error" ? window.red : window.vpnAccent
            border.width: 1
            Behavior on border.color { ColorAnimation { duration: 200 } }
            ...
            Column {
                ...
                // Error message
                Rectangle {
                    visible: window.vpnErrorMsg !== ""
                    width: 280; height: 28
                    radius: 6
                    color: Qt.rgba(window.red.r, window.red.g, window.red.b, 0.13)
                    Text {
                        anchors.centerIn: parent
                        text: window.vpnErrorMsg
                        font.family: "JetBrains Mono"
                        font.pixelSize: 11
                        color: window.red
                    }
                ...
                Rectangle {
                    ...
                    radius: 8
                    color: window.surface0
                    border.color: vpnUserInput.activeFocus ? window.vpnAccent : window.surface2
                    border.width: 1
                    Behavior on border.color { ColorAnimation { duration: 150 } }
                ...
                Rectangle {
                    ...
                    radius: 8
                    color: window.surface0
                    border.color: vpnPassInput.activeFocus ? window.vpnAccent : window.surface2
                    border.width: 1
                    Behavior on border.color { ColorAnimation { duration: 150 } }
                    ...
                    TextInput {
                        ...
                        Keys.onReturnPressed: vpnConnectBtn.doConnect()
                    }
                }
                ...
                Row {
                    ...
                    Rectangle {
                        width: 90; height: 36
                        radius: 10
                        color: vpnCancelMa.containsMouse ? window.surface2 : window.surface1
                        Behavior on color { ColorAnimation { duration: 150 } }
                        Text {
                            anchors.centerIn: parent
                            text: "Cancel"
                    ...
                    Rectangle {
                        ...
                        width: 90; height: 36
                        radius: 10
                        color: vpnConnectMa.containsMouse ? Qt.lighter(window.vpnAccent, 1.1) : window.vpnAccent
                        Behavior on color { ColorAnimation { duration: 150 } }

                        function doConnect() {
                            let user = vpnUserInput.text.trim();
                            let pass = vpnPassInput.text.trim();
                            if (user === "" || pass === "") return;
                            window.showVpnDialog = false;
                            window.vpnErrorMsg = "";
                            let cmd = "nmcli connection modify 'Ensimag-VPN-ETU-udp' vpn.user-name '" + user + "' +vpn.data 'password-flags=0' +vpn.secrets 'password=" + pass + "' && nmcli connection up 'Ensimag-VPN-ETU-udp' 2>&1 && echo __VPN_OK__ || echo __VPN_FAIL__";
                            vpnProcess.command = ["bash", "-c", cmd];
                            vpnProcess.running = true;
                        }
                        ...
                        MouseArea {
                            ...
                            anchors.fill: parent
                            hoverEnabled: true
                            onClicked: vpnConnectBtn.doConnect()
                        }
                    }
                }
            }
