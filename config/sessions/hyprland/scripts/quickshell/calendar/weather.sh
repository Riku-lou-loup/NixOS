...
get_location() {
    ...
    loc=$(curl -sf "https://ipinfo.io/json" 2>/dev/null)
    if [ -z "$loc" ]; then
...
get_data() {
    if [[ -z "$KEY" || "$KEY" == "Skipped" || "$KEY" == "OPENWEATHER_KEY" ]]; then
        ...
        final_json="${final_json%,}]"
        echo "{ \"city\": \"Unknown\", \"forecast\": ${final_json} }" > "${json_file}"
        return
    fi
    ...
    if [ -z "$LAT" ] || [ "$LAT" = "null" ]; then return; fi
    ...
    CITY_NAME=$(echo "$raw_api" | jq -r ".city.name")
    CITY_COUNTRY=$(echo "$raw_api" | jq -r ".city.country")
    ...
    if [ ! -z "$processed_forecast" ]; then
        ...
        echo "{ \"city\": \"${CITY_NAME}, ${CITY_COUNTRY}\", \"forecast\": ${final_json} }" > "${json_file}"
    fi
}
